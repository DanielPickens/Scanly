...
func (cmdp *Comparer) DeleteCache() (errors []error) {
	for key := range cmdp.trees {
		pathError, _ := cmdp.GetPathErrors(key)
		if len(pathError) > 0 {
			for _, path := range pathError {
				errors = append(errors, fmt.Errorf("path error at layer index %s: %s", key, path))
			}
		}
		err := cmdp.DeleteTree(key)
		if err != nil {
			errors = append(errors, err)
			return errors
		}
	}
	for key := range cmdp.pathErrors {
		err := cmdp.DeletePathErrors(key)
		if err != nil {
			errors = append(errors, err)
			return errors
		}
